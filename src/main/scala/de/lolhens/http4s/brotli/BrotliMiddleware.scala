...
object BrotliMiddleware {
  def decompress[F[_] : Async: BrotliDecompressor](message: Message[F],
    ...
    message.headers.get[`Content-Encoding`].map(_.contentCoding) match {
      case Some(ContentCoding.br) =>
        message
          .withBodyStream(message.body.through(BrotliDecompressor[F].decompress))
  ...
  def apply[F[_] : Async: BrotliDecompressor](routes: HttpRoutes[F],
